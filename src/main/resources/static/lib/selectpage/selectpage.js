...
; (function ($) {
    ...
    SelectPage.prototype.setElem = function (combo_input) {
        ...
        // modified by Liang @2018-04-11 将下拉内容加载至与控件同一界面
        $(combo_input).parent().append(elem.result_area);
    ...
    SelectPage.prototype.eInput = function () {
        ...
        el.combo_input.keyup(function (e) {
            ...
            if (el.result_area.is(':hidden')) {
                ...
                $(el.combo_input).val(self.prop.selected_text);
            }
        });
	...
	/*
	 * Add By Liang @2018-04-11 选中控件的默认值
	 */
    SelectPage.prototype.SetSelectedValue = function (combo_input) {
        var keyField = this.option.keyField;
        var showField = this.option.showField;

        var selectedText = '';
        var selectedValue = this.option.selectedValue;
        $(this.option.data).each(function () {
            if (this[keyField] === selectedValue) {
                $(combo_input.nextElementSibling).val(selectedValue);
                $(combo_input).val(this[showField]);

                selectedText = this[showField];
            }
        });
        this.prop = this.prop || {};
        this.prop.selected_text = selectedText;
    };
